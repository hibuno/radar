...
async function processHandler(_request: NextRequest) {
  try {
    console.log("🔄 Starting repository processing (images and metadata)...");
    ...
    // Get repositories that need processing (empty images, missing metadata, etc.)
    ...
    console.error("💥 Processing automation error:", error);
    return NextResponse.json(
      {
        success: false,
        error:
