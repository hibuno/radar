	...
	try {
		...
		articleElements.forEach(article => {
			try {
				...
				// Extract thumbnail
				const thumbnailImg = article.querySelector('img[src*="social-thumbnails"]');
				...
				// Extract upvotes
				const upvoteElement = article.querySelector('.flex.items-center.flex-col .leading-none');
				...
				const submittedByContainer = Array.from(article.querySelectorAll('*')).find(el =>
					el.textContent?.includes('Submitted by')
				);
				...
				const authorCountText = Array.from(article.querySelectorAll('*')).find(el =>
					el.textContent?.includes('authors')
				)?.textContent || '';

				const authorCountMatch = authorCountText.match(/(\d+)\s+authors/);
				authorCount = authorCountMatch ? parseInt(authorCountMatch[1]) : authors.length;

				// Extract GitHub count (first metric)
				const metricElements = article.querySelectorAll('.flex.items-center.gap-2 a span, .flex.items-center.gap-2 span');
				let githubCount = 0;
				let commentCount = 0;

				if (metricElements.length >= 1) {
					githubCount = parseInt(metricElements[0]?.textContent?.trim() || '0') || 0;
				}

				// Extract comment count (look for the blue-colored element which typically indicates comments)
				const commentElement = article.querySelector('[class*="text-blue-500"], [class*="bg-blue-600"]');
				if (commentElement) {
					const commentText = commentElement.textContent?.trim() || '0';
					commentCount = parseInt(commentText) || 0;
				} else if (metricElements.length >= 2) {
					commentCount = parseInt(metricElements[1]?.textContent?.trim() || '0') || 0;
				}
