...
export async function GET(request: Request) {
	try {
		...
		// Clean up title
		paperDetail.title = (titleElement?.textContent?.trim() || 'N/A').replace(/\s+/g, ' ');
		...
		const paperOfDayElement = Array.from(document.querySelectorAll('.text-blue-500')).find(el =>
			el.textContent?.includes('Paper of the day'));
		if (paperOfDayElement) {
			paperDetail.paperOfTheDay = paperOfDayElement.textContent?.trim();
		}

		// Extract upvotes
		const upvoteElement = document.querySelector('.font-semibold.text-orange-500') ||
			document.querySelector('.text-orange-500');
		if (upvoteElement) {
			paperDetail.upvotes = parseInt(upvoteElement.textContent?.trim() || '0') || 0;
		}

		// Extract authors
		const authorElements = document.querySelectorAll('.author, span.author');
		authorElements.forEach(authorEl => {
			const nameElement = authorEl.querySelector('a') || authorEl.querySelector('button');
		...
		if (paperDetail.authors.length === 0) {
			const authorsSection = Array.from(document.querySelectorAll('*')).find(el =>
				el.textContent?.includes('Authors:'));

			if (authorsSection) {
				const authorLinks = authorsSection.parentElement?.querySelectorAll('a[href*="/"]') || [];
		...
		const abstractSection = Array.from(document.querySelectorAll('h2')).find(h2 =>
		...
		if (abstractSection) {
			...
			const aiSummaryElement = abstractContainer?.querySelector('[class*="bg-blue-"], [class*="border-blue-"]');
			if (aiSummaryElement) {
				const summaryText = aiSummaryElement.querySelector('p');
				paperDetail.aiSummary = summaryText?.textContent?.trim().replace(/\s+/g, ' ');
			}

			// Extract main abstract (gray text paragraph)
			const abstractElement = abstractContainer?.querySelector('.text-gray-600, p.text-gray-600');
			if (abstractElement) {
				paperDetail.abstract = abstractElement.textContent?.trim().replace(/\s+/g, ' ') || 'N/A';
			}
		}
